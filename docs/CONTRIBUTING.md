...
The parser reads `:doc`, `:private` and `:macro` from both places a form can carry metadata - the meta-map after the name and the `^…` tags before it - so `(def defn {:macro true} …)` is recorded as a macro and `(def ^:private symbol-munge …)` as private, exactly as phel means them.

`src/phelCoreSymbols.ts` exposes flat name arrays projected from this database (`MACROS`, `CORE_FNS`, `CORE_VALUES`) plus a hand-curated `SPECIAL_FORMS` for the engine special forms (which live in PHP, not in any `.phel` file) and a hand-curated `CORE_DEF_FORMS` for the bootstrap `(def …)` names that hold a function rather than a constant, which is the one thing no marker records - see [completion.md](completion.md#completion).
