...
| `phel.diagnostics.live` | boolean | `true` | Also report analyzer diagnostics as you type, through a long-lived `phel api-daemon`. The same daemon indexes the project for namespace-aware go-to-definition and cross-file references and answers PHP-interop completion, so this switch turns those off too. See [Live diagnostics](#live-diagnostics), [what the daemon adds to navigation](refactoring.md#what-the-analysis-daemon-adds) and [PHP interop](completion.md#php-interop). |
...
| `phel.migration.enabled` | boolean | `true` | Flag what Phel 0.50 removed (core aliases, `#\| \|#`, bare `#` comments, `\|()` short fns, `foo$` gensyms, `,` unquote, `^:reference`) and what 0.52 removed (`php/new`, `php/->`, `php/::`, `set-var`), plus the deprecated `\` namespace separator and calls to your own `:deprecated` definitions, with a quick fix where the rewrite is mechanical. Turn off when targeting a Phel older than 0.52. Severity follows the project, see [below](#what-the-project-config-decides). See [Migration hints](completion.md#migration-hints). |
