...
| `phel.migration.enabled` | boolean | `true` | Flag calls to core functions Phel 0.50 removed and to the forms it deprecated as source, with a quick fix where the replacement is a plain rename. Turn off when targeting a Phel older than 0.50. See [Migrating to Phel 0.50](completion.md#migrating-to-phel-050). |
