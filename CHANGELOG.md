...
- Four ways to keep what an evaluation returned. **Phel: nREPL Evaluate to Comment** (`ctrl+alt+c` / `cmd+alt+c`) writes the value under the form as `;; => …`, and evaluating that form again rewrites the block instead of stacking a second one under the first — a multi-line value gets one comment line each, padded so the value stays in one column, which is also what tells the block apart from an ordinary `;; note` written under a result. **Phel: nREPL Evaluate and Replace Form** swaps the form for its value, and refuses on an error result rather than replacing the only copy of the form with a stack trace. **Phel: nREPL Show Last Result** opens `phel-result:last.phel` beside the editor — a read-only document, Phel-highlighted because a Phel value *is* Phel data, refreshed by every eval command, so a pinned second column becomes a result pane. And **Phel: REPL History** reads `.vscode/phel-repl-history.phel` back as a picker (newest first, one row per distinct form), sending the picked form to the REPL terminal, or over the nREPL connection through a per-row button that only appears while one is live.
- **Phel: Show Documentation** now opens a searchable **Phel API** panel — one webview per window, reused — instead of a Markdown preview tab: the symbol at the cursor preselected, a search box filtering the ~900 public symbols of the bundled corpus by name and docstring, and a detail pane with the example, the see-also list, a **View source** link and the namespace's page on phel-lang.org (the page, not an anchor: the site's per-symbol anchors are de-duplicated slugs no corpus can derive). The page is built at runtime and loads nothing — `default-src 'none'`, one nonced `<style>` and one nonced `<script>` — and every docstring is escaped, since they are source text. Where a window cannot host a webview the command falls back to the Markdown preview it used before, and it now answers `{ symbol }` with what it showed.
- Hovering a `php/<fn>` call shows the signature that PHP actually has for it — `strtoupper(string $string): string` — reflected through the analysis daemon (`phel.diagnostics.live`), plus a link to the manual page. There is deliberately no bundled prose: php.net's descriptions are CC-BY and move with every release, while a reflected signature comes from the interpreter that will run the code. A hover cannot wait for a PHP boot, so the first one against a cold daemon shows the link alone and the request that started it fills a session cache the next hover reads; without a daemon the link is the whole hover. The interop special forms (`php/new`, `php/aget`, …) and the superglobals keep the hovers they had.

- Conditional breakpoints, hit counts, logpoints and a working watch panel in the debugger, plus **Phel: Debug Test** (`phel.debugTest`) — a `$(debug-alt) Debug test` CodeLens above every `deftest`, an entry in the editor's Phel menu, and a Debug profile in the Testing view — which starts a session on a free port and runs `phel test --filter '/^<name>$/' <file>` with `XDEBUG_MODE=debug`, `XDEBUG_SESSION=1` and `XDEBUG_CONFIG=client_port=<port>` so the run dials back into it. A condition is written in Phel and translated to the PHP Xdebug evaluates, with names munged exactly as the compiler emits them (`item-count > 3` → `$item_count > 3`, `blank?` → `$blank_QMARK_`, `:done` → `Keyword::create("done")`, `nil` → `null`); anything already PHP is passed through, a hit count becomes DBGp's `-h`/`-o`, and a logpoint prints its `{expression}`s from the paused frame and resumes without stopping. A breakpoint the engine refuses is left unverified with what it said (a condition that is merely invalid PHP is accepted by Xdebug and then silently never fires — it is evaluated later, not at set time), and a watch or hover expression that is out of scope reads `<not available>` instead of an error dump (the Debug Console still gets the reason). `docs/debugging.md` has the details, including the by-hand checklist for what no suite can observe: a breakpoint actually being hit.

- The status-bar item now says what the Phel processes behind the editor are doing: one icon per subsystem that is up — `$(pulse)` the analysis daemon, `$(plug)` an nREPL connection, `$(server)` the language server — with the state of all three, per workspace folder, in its tooltip. Clicking it opens **Phel: Status Actions** (`phel.status.actions`) instead of starting a REPL straight away: start a REPL, connect or disconnect the nREPL server (whichever the current state allows), restart the analysis daemon or the language server, show the **Phel Analysis** output, run doctor. The states come from one hub every subsystem publishes into (`src/phelRuntimeState.ts`), so nothing depends on which of them was created first, and a state set to what it already is fires nothing — the daemon reports around every keystroke pause.

- PHP-interop completion, answered by the compiler (`phel.completion.phpInterop`, default on). Typing `(php/-> receiver …)`, `(php/:: Class …)`, `\Class/member`, `(.method x)`, `(php/new \Foo`, a bare `\Foo`, `php/strto…` or `php/$_SE…` now offers what PHP actually has there — the class's public methods, properties, constants and static properties, the classes on the project's classmap, the global functions with their signatures, the superglobals — through the same one-process-per-folder `phel api-daemon` live diagnostics use (`phel.diagnostics.live`). The receiver's class is resolved from a `(php/new …)` binding, a `^{:tag \Foo}` annotation, a `(:use …)` import or the return type of a method the form hops through. It is a second completion provider next to the bundled one, so both lists show; the daemon gets 400 ms per keystroke and a busy or still-booting one simply contributes nothing to that popup. A Phel without `api-daemon` gets exactly what it had.

- Namespace hygiene. A `(:require …)` entry nothing in the file uses is faded as you type, with a **Remove unused require '…'** quick fix that also drops the `(:require …)` clause when that was its last entry — the same rule as `phel lint`'s `phel/unused-require` (alias from `:as` or the namespace's last segment, so `(:require phel.json)` is used by `json/encode` and nothing else), plus the per-name hint a lint rule cannot give: `[app.core :refer [greet shout]]` with only `greet` called offers **Remove unused refer 'shout'**. A use inside a syntax-quoted macro template counts, deliberately. After a save the CLI's warning replaces the hint on the same entry, and the quick fix answers either. **Source Action → Sort requires** orders the entries by namespace without moving the whitespace between them, so `"source.organizeImports"` on save is idempotent. **Phel: Go to Test / Source File** (`phel.ns.goToTest`) walks between `src/strings.phel` and `tests/strings_test.phel` following the project's `src-dirs` / `test-dirs` and Phel's own `<ns>-test` convention, offering to scaffold the test file when there is none. And a newly created, still empty `.phel` file gets its `(ns …)` form (`phel.ns.autoInsert`, default on), with the namespace read off how the neighbouring files map their own paths — `phel config` does not print the project's main namespace, so nothing else can tell `demo.strings` from `app.strings`.

- A `N references` CodeLens above every definition, clicking through to the same list <kbd>shift</kbd>+<kbd>F12</kbd> shows (`phel.references.codeLens`, default on, per folder). The count is a map lookup, not a search: the workspace index now tallies the symbol tokens of each file as it reads it, so a lens costs nothing per keystroke, and an unsaved buffer is re-counted from what it says now. `deftest` forms get none — nothing calls them by name, and they already carry a **Run test** lens. Find All References and Rename now also see a use written through an alias: `s/shout` is one token, so a scan for `shout` could never match inside it, and the hits the daemon reported for one were spanned as if the alias were part of the name — fatal for a rename, which now rewrites only the name half and leaves `s/` alone.
...
- On-type indentation (`phel.format.onType`, on by default): pressing <kbd>Enter</kbd> puts the new line exactly where `phel format` would, and a closing bracket typed at the start of a line re-places that line. The rules mirror phel-lang's own `IndentRule` — the two-space body of `defn` / `fn` / `deftest` and friends, the block forms that switch to two spaces once their body starts a line (`if`, `let`, `cond`, `try`, `ns`, `catch`, …), and everything else aligned under a call's first argument, which is what lines `->` steps up — so format-on-save never has to move a line again. Over phel's own stdlib the two agree on 16374 of 16375 lines (`npm run sweep`). The extension turns `editor.formatOnType` on for `.phel` files, since VS Code ships it off; the `indentationRules` in `language-configuration.json`, which counted brackets rather than reading forms, are gone.
...
- A definition made private with metadata rather than with a `-` suffix is now indexed as private. `(def ^:private secret …)` and `(defn foo {:private true} …)` are exactly as private as `(def- …)` / `(defn- …)`, but the parser read `private` off the defining operator alone — so every such definition in a workspace was offered to every other namespace by completion, listed under its `:refer [ … ]`, and shown without the `private` detail in the outline. `:private` is now read from the meta-map after the name and from the `^…` tags before it, both places phel accepts it.

- Definitions that document themselves with `{:doc "…"}` now have their docs in hover, completion and signature help. The parser only took a docstring from a string literal after the name, and `:doc` in a meta-map is the other spelling phel writes — 28 corpus entries shipped without the docs their source carries, `first`, `next`, `with-meta`, `defn`, `declare` and `hash-map` among them. `{:macro true}` is read the same way, so the macros core installs with a bare `(def defn {:macro true} …)` are now macro records in the corpus rather than hand-listed in `CORE_DEF_FORMS`, and the fifteen internal helpers core marks `{:private true}` / `^:private` stay out of completion on their own.

- Completion now offers the `phel.core` names that core installs with a bare `(def …)`: the macros `defn`, `defmacro`, `declare` and `meta`, the functions written before `defn` exists to write them (`first`, `next`, `with-meta`, `vary-meta`, `queue`, `array-map`, `to-php-array`), and the values `*ns*`, `*file*`, `*argv*`, `*program*`, `*assert*` and `NAN`. The symbol corpus took `kind` from the defining operator, so all of them read as plain `def`s and fell through both the macro and the function projection — `defn` only looked right because a snippet of the same name exists, and `first` was missing outright. `CORE_DEF_FORMS` in `src/phelCoreSymbols.ts` carries the classification no marker in the source records: that such a `def` holds a function rather than a constant.

- A debug session no longer stops in vendor code before reaching yours. The **Uncaught Exceptions** filter — on by default — installed `breakpoint_set -t exception -x Exception`/`-x Error`, and DBGp breaks where an exception is *thrown*, caught or not: traced against Xdebug 3.4, a `phel test` run stopped inside the console component's own `try`/`catch` every time. The same trace shows an uncaught exception with no exception breakpoint does not stop at all, so "uncaught only" is not something Xdebug can express. The filter is gone; **All Exceptions** stays, off by default, and says in its description that caught ones are included.

- Running a single test now runs it. `phel.runTest` and the `▶ Run test` CodeLens passed `--filter '^name$'`, and `phel test` treats a filter that is not wrapped in PCRE delimiters as a literal substring (`preg_quote`) — so the anchored form matched nothing and the run reported "No tests matched the given paths or selectors". The name goes in as `/^name$/` now.

- Evaluating a keyword in a paused debug session no longer dies. `:status` was sent to Xdebug as `new \Phel\Lang\Keyword("status")`, and that constructor is private — the eval returned a fatal error instead of the keyword. It goes through `Keyword::create` now.

- Running a `phel` task no longer wipes the extension's own squiggles. `$phel-lint` and `$phel-test-watch` filed their problems under the marker owner `phel`, which is also the name of the diagnostic collection the on-save `phel lint` pass writes to — and a task run replaces, then on the next run clears, every marker under its owner, so the on-save findings disappeared as soon as **Run Task… → phel: lint** finished (and came back only on the next save). Both matchers now own `phel-tasks`; `source` stays `phel`, so the Problems panel is unchanged.

- A `(ns …)` form with more than one `(:require …)` clause — which is what `phel init` scaffolds, one clause per namespace — is now read whole. Only the first clause was, so an `:as` alias declared in a second one resolved nowhere (hover, go to definition, signature help, completion and inlay hints all fall back to that alias map), a `:refer`'d name from one was not attributed to its namespace, the namespace-completion list offered a namespace already required, and auto-import appended a duplicate entry for it. Over phel's own stdlib that was 19 of 40 require entries seen.
...
- A workspace folder can now carry its own CLI path. No contributed setting declared a `scope`, which makes every one of them window-scoped, and VS Code then ignores a value written in a folder's `.vscode/settings.json` — so `phel.executablePath` and the per-command overrides were resolved per folder by code that could never see a folder value. `phel.executablePath`, `phel.diagnostics.command`, `phel.format.command`, `phel.test.command`, `phel.repl.command`, `phel.lsp.command`, `phel.lsp.args`, `phel.repl.args`, `phel.repl.history.enabled`, `phel.nrepl.reloadOnSave` and `phel.nrepl.hoverEval` are now `resource`-scoped: in a multi-root workspace each project points at its own binary, and a file's own folder decides. Everything else stays window-scoped, either because it gates what activation registers or because the extension reads it once for the window; `docs/settings.md` lists which is which.
...
- The Marketplace listing now says what the extension is: `categories` gains **Linters**, **Formatters** and **Testing** next to Programming Languages, Snippets and Debuggers, `keywords` gains the seven terms someone looks for it by (`repl`, `nrepl`, `paredit`, `structural editing`, `test explorer`, `interop`, `language server`), and the one-line `description` — the subtitle under the name, which still read "syntax highlighting, code completion, and snippets" — names the same things the categories now claim. Screenshots are still absent, and deliberately not faked: the extension host has no screenshot API and every frame worth showing is an overlay that closes the moment automation takes focus, so `docs/media/README.md` is the reproducible recipe for the three by-hand shots (window 1280×800, Dark Modern, zoom 1, which project and which keystroke for each) and `scripts/make-gif.sh` assembles captured frames into a two-pass-palette GIF. `packageManifest.test.ts` now checks the categories against the Marketplace's list, the 30-keyword cap and their uniqueness, and that every image the README names exists — `vsce` serves README images off the default branch, so a wrong path is broken on the listing and nowhere else.
