...
- Completion now offers the `phel.core` names that core installs with a bare `(def …)`: the macros `defn`, `defmacro`, `declare` and `meta`, the functions written before `defn` exists to write them (`first`, `next`, `with-meta`, `vary-meta`, `queue`, `array-map`, `to-php-array`), and the values `*ns*`, `*file*`, `*argv*`, `*program*`, `*assert*` and `NAN`. The symbol corpus takes `kind` from the defining operator, so all of them read as plain `def`s and fell through both the macro and the function projection — `defn` only looked right because a snippet of the same name exists, and `first` was missing outright. `CORE_DEF_FORMS` in `src/phelCoreSymbols.ts` carries that classification, and keeps out the fifteen helpers core marks `:private` in a meta-map the corpus does not carry.
