...
- `MACROS` and `CORE_FNS` in `src/phelCoreSymbols.ts` derive from the corpus automatically; `SPECIAL_FORMS` is hand-curated (engine forms live in PHP, not in any `.phel` file) — add new special forms there by hand and mirror them in the `corelib` list of `syntaxes/phel.tmLanguage.json`.
